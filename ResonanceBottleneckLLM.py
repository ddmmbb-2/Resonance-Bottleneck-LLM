...
class ResonanceMemoryAttentionV15(nn.Module):
    ...
    def forward(self, x):
        ...
        decay_rate = 0.1 + 0.85 * torch.sigmoid(decay_raw) 
        ...
        # 🟢 修正 1：限制 cum_log_decay 的下限，防止數值過小導致後續運算爆炸
        cum_log_decay = torch.clamp(cum_log_decay, min=-20.0)
        
        decay_factor = torch.exp(cum_log_decay) # [B, L, H]
        
        # 🟢 修正 2：計算反向衰減因子，並限制上限以代替危險的除法
        inv_decay_factor = torch.exp(-cum_log_decay).clamp(max=1e4)
        
        # 擴展維度以便矩陣運算
        df_kv = decay_factor.unsqueeze(-1).unsqueeze(-1)
        idf_kv = inv_decay_factor.unsqueeze(-1).unsqueeze(-1) # 準備給 kv 使用
        
        df_z = decay_factor.unsqueeze(-1)
        idf_z = inv_decay_factor.unsqueeze(-1) # 準備給 z 使用
        ...
        # 分子累積 (Memory State) -> ⚠️ 將原本的 / (df_kv + 1e-6) 改為 * idf_kv
        kv_states = torch.cumsum(kv_input * idf_kv, dim=1) * df_kv
        kv_states = kv_states.transpose(1, 2) # [B, H, L, d, d]
        
        # 分母累積 (Normalizer) -> ⚠️ 將原本的 / (df_z + 1e-6) 改為 * idf_z
        z_input = k_f * (1.0 - decay_rate).unsqueeze(-1)
        z_states = torch.cumsum(z_input * idf_z, dim=1) * df_z # [B, L, H, d]

        # 🟢 修正 3：限制 z_states 的上下限，防止分母累積出異常極值傳遞給最終輸出
